...
@ray.remote
def f (words, sizex, y, best, moves_d, us, them, reverse, owned):
...
def minmax(depth, owned, reverse=False, moves=[]):
  ...
  if depth>1:
    futures = [f.remote(words, sizex, y, best, moves_d, us, them, reverse, owned) for y in loopgen]
    better = ray.get(futures)
    for b in better:
      if b[0]>best[0]:
        best=b
  else:
    b = f(words, sizex, y, best, moves_d, us, them, reverse, owned)
    if b[0]>best[0]:
      best=b
